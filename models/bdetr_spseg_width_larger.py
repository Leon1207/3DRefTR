...
class BeaUTyDETR_spseg_width_larger(nn.Module):
    """
    ...
    def __init__(self, num_class=256, num_obj_class=485,
        ...
        if contrastive_align_loss:
            self.contrastive_align_projection_image_larger = nn.Sequential(
            ...
            self.contrastive_align_projection_text_larger = nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.ReLU(),
                nn.Linear(d_model, d_model),
                nn.ReLU(),
    ...
    def forward(self, inputs):
        ...
        if self.contrastive_align_loss:
            proj_tokens = F.normalize(
                self.contrastive_align_projection_text_larger(text_feats), p=2, dim=-1
            )
        ...
        if self.contrastive_align_loss: 
            end_points['proposal_proj_queries'] = F.normalize(
                self.contrastive_align_projection_image_larger(query), p=2, dim=-1
            )  # [B, 256, 64]
