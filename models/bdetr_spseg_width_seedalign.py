...
class BeaUTyDETR_spseg_width_seedalign(nn.Module):
    """
    ...
    def __init__(self, num_class=256, num_obj_class=485,
        ...
        if contrastive_align_loss:
            ...
            self.contrastive_align_projection_seed = nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.ReLU(),
                nn.Linear(d_model, d_model),
                nn.ReLU(),
    ...
    def forward(self, inputs):
        ...
        if self.contrastive_align_loss: 
            ...
            end_points['proj_seeds'] = F.normalize(
                self.contrastive_align_projection_seed(points_features.transpose(1, 2).contiguous()), p=2, dim=-1
            )  # [B, 1024, 64]
