...
class HungarianMatcher_maskalign(nn.Module):
    """
...
# BRIEF Compute loss
class SetCriterion_maskalign(nn.Module):
    ...
    def loss_sem_align_selection(self, outputs, targets, indices, num_boxes, auxi_indices):

        if "proj_seeds" not in outputs:
            return {"loss_sem_align_selection": 0.0}

        tokenized = outputs["tokenized"]

        # step 1. Contrastive logits
        norm_text_emb = outputs["proj_tokens"]  # B, num_tokens=L, dim=64
        norm_img_emb = outputs["proj_seeds"]  # B, num_queries=1024, dim=64
        logits = (
            torch.matmul(norm_img_emb, norm_text_emb.transpose(-1, -2))
            / self.temperature
        )  # [[B, num_queries, num_tokens]
        ...
        # total loss
        tot_loss = (box_to_token_loss + token_to_box_loss) / 2
        return {"loss_sem_align_selection": tot_loss / num_boxes}
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, auxi_indices, **kwargs):
        loss_map = {
            ...
            'selection': self.loss_sem_align_selection
        }
...
# BRIEF loss
def compute_hungarian_loss_maskalign(end_points, num_decoder_layers, set_criterion,
    ...
    loss_ce, loss_bbox, loss_giou, loss_sem_align, loss_mask, loss_dice, loss_sem_align_selection = 0, 0, 0, 0, 0, 0, 0
    for prefix in prefixes:
        ...
        loss_sem_align_selection += losses['loss_sem_align_selection']
        if 'proj_tokens' in end_points:
    ...
    loss = (
        ...
        + 1.0 / (num_decoder_layers + 1) * (
            ...
            + weight * loss_sem_align_selection
    ...
    end_points['loss_sem_align_selection'] = loss_sem_align_selection
